...
<template>
      ...
      <div class="dialog-container raster-preview-layout">
        <div class="preview-main">
          ...
          <div class="raster-actions" v-if="selectedData.data_id">
            ...
            <el-button type="warning" plain size="small" :loading="generatingTiles" @click="generateTilesForPreview">{{ $t('rasterWmEmbed.generateTiles') }}</el-button>
          </div>
        </div>
  ...
  const generateTilesForPreview = async () => {
    if (!selectedData.data_id) return;
    generatingTiles.value = true;
    try {
      const resp = await generateRasterTiles({ data_id: selectedData.data_id });
      if (resp.data?.status) {
        ElMessage.success(resp.data.msg || t('rasterWmEmbed.tilesGenerated'));
      } else {
        ElMessage.error(resp.data?.msg || t('rasterWmEmbed.tilesGenerateFailed'));
      }
    } catch (e) {
      console.error('Tile generation failed:', e);
      ElMessage.error(t('rasterWmEmbed.tilesGenerateFailed'));
    } finally {
