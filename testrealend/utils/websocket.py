...
def init_socketio(app):
    ...
    try:
        ...
        _socketio = SocketIO(
            app, cors_allowed_origins=cors_origins, async_mode="threading", logger=False, engineio_logger=False
        )
...
def _register_handlers(sio, app):
    ...
    def handle_authenticate(data):
        ...
        emit("authenticated", {"status": "ok", "user_number": user_number, "role": role})
...
def notify_new_chat_message(
    message_id, sender_number, sender_role, receiver_number, receiver_role, content, created_at
):
