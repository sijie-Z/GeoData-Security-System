...
def to_geodataframe(dataframe, index, coor_nested, shpfile_type):
    ...
    if shpfile_type == "Point":
        ...
        if x_coords.size == 0:
            dataframe.loc[index, "geometry"] = None
        elif isinstance(x_coords[0], (list, np.ndarray)):
            # Polygon with holes (parts array from nested geometry processing)
            exterior = _convert_coords_to_decimal(zip(x_coords[0], y_coords[0]))
            holes = []
            for h in range(1, len(x_coords)):
                if x_coords[h].size > 2:
                    holes.append(_convert_coords_to_decimal(zip(x_coords[h], y_coords[h])))
            dataframe.loc[index, "geometry"] = Polygon(exterior, holes=holes if holes else None)
        elif x_coords.size > 2:
