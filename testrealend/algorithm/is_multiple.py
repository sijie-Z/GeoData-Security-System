    ...
    try:
        ...
        if indices:
            ...
            for idx in indices:
                ...
                if geom is None:
                    continue
                if (isinstance(geom, MultiPolygon) and len(geom.geoms) > 0) or (
                    isinstance(geom, MultiLineString) and len(geom.geoms) > 0
                ):
            ...
            logging.info(
                "共转换 %d 条多部分几何为单部分，对应索引号为：%s，类型为：%s",
                converted,
                indices,
                list(set([feature_type[i] for i in indices])),
            )
