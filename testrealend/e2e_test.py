...
def test_raster_extract(token):
    ...
    resp = requests.get(
        f"{BASE_URL}/api/adm2_embedding_watermark_applications",
    ...
    if not target_app.get("watermark_generated"):
        ...
        resp = requests.post(
            f"{BASE_URL}/api/generate_watermark",
    ...
    resp = requests.post(
        f"{BASE_URL}/api/admin/embed_dispatch",
        json={"application_id": app_id},
        headers={"Authorization": f"Bearer {token}"},
    )
    ...
    with open(stego_path, "rb") as f:
        ...
        resp = requests.post(
            f"{BASE_URL}/api/vector/extract", files=files, data=data, headers={"Authorization": f"Bearer {token}"}
        )
...
def test_watermark_comparison(token):
    ...
    if apps:
        ...
        resp = requests.post(
            f"{BASE_URL}/api/get_original_watermark",
            json={"number": app_id},
            headers={"Authorization": f"Bearer {token}"},
        )
