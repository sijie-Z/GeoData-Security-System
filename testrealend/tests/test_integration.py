...
class TestApprovalWorkflow:
    ...
    def test_full_approval_flow(self, client, auth_headers, adm2_headers, employee_headers, db):
        ...
        resp = client.post(
            ...
            json={
                "data_id": 100,
                "data_name": "Test Vector",
                "data_alias": "test_vec",
                "data_url": "/data/test.shp",
                "data_type": "vector",
                "applicant_name": "员工1",
    ...
    def test_rejection_at_adm1(self, client, auth_headers, employee_headers, db):
        resp = client.post(
            ...
            json={
                "data_id": 101,
                "data_name": "Reject Test",
                "data_alias": "reject",
                "data_url": "/data/reject.shp",
                "data_type": "vector",
                "applicant_name": "员工1",
                "applicant_user_number": "E001",
                "reason": "test",
...
class TestAdminRoleEnforcement:
    ...
    def test_employee_cannot_send_notification(self, client, employee_headers, db):
        resp = client.post(
            "/api/admin/notifications/send", json={"title": "test", "content": "test"}, headers=employee_headers
        )
        assert resp.status_code == 403
...
class TestTokenBlacklist:
    ...
    def test_logout_blacklists_token(self, client, db):
        resp = client.post("/api/login", json={"username": "admin1", "password": "admin123", "role": "admin"})
        assert resp.status_code == 200
...
class TestPasswordValidation:
    ...
    def test_register_rejects_weak_password(self, client, db):
        resp = client.post("/api/register", data={"name": "Weak", "employeeId": "W001", "password": "123"})
        assert resp.status_code == 400

    def test_register_rejects_no_digits(self, client, db):
        resp = client.post("/api/register", data={"name": "NoDigit", "employeeId": "W002", "password": "abcdefgh"})
        assert resp.status_code == 400
...
class TestUserProfile:
    ...
    def test_update_profile(self, client, employee_headers, db):
        resp = client.put(
            ...
            data={"userName": "Updated Name", "email": "test@example.com"},
            headers=employee_headers,
...
class TestInputSanitization:
    ...
    def test_chat_message_length_limit(self, client, employee_headers, db):
        resp = client.post(
            "/api/chat/send",
            json={"receiver_number": "admin1", "receiver_role": "admin", "content": "x" * 2001},
            headers=employee_headers,
        )
        assert resp.status_code == 400
...
class TestVectorWatermarkLifecycle:
    ...
    def _create_test_shapefile(tmp_dir):
        ...
        with fiona.open(shp_path, "w", driver="ESRI Shapefile", schema=schema, crs="EPSG:4326") as dst:
            ...
            for i in range(130000):
                ...
                coords.append((x, y))
            dst.write({"geometry": {"type": "LineString", "coordinates": coords}, "properties": {"id": 1}})
        return shp_path
    ...
    def test_full_vector_lifecycle(self, client, auth_headers, adm2_headers, employee_headers, db):
        ...
        resp = client.post(
            ...
            json={
                "data_id": shp_id,
                "data_name": "Vector WM Test",
                "data_alias": unique_alias,
                "data_url": shp_path,
                "data_type": "vector",
                "applicant_name": "员工1",
                "applicant_user_number": "E001",
        ...
        # ── Step 4: 生成水印 QR ──
        resp = client.post(
            "/api/generate_watermark",
            json={"application_id": app_id, "purpose": "测试", "usage_scope": "内部"},
        ...
        resp = client.post(
            "/api/embedding_watermark", json={"application_id": app_id, "algorithm": "lsb"}, headers=adm2_headers
        )
        ...
        if resp.status_code == 200:
            ...
            print("  [5/6] 嵌入API可达，numpy 2.0兼容性问题（预期行为）")

        # ── Step 6: 验证下载令牌流程 ──
        resp = client.post("/api/request_download_token", json={"application_id": app_id}, headers=employee_headers)
