...
class TestApprovalWorkflow:
    ...
    def test_adm1_pass_nonexistent(self, client, auth_headers):
        resp = client.post(
            "/api/adm1_pass", headers=auth_headers, json={"id": 99999, "user_name": "Admin", "user_number": "admin1"}
        )
        assert resp.status_code in (404, 400)

    def test_adm1_fail_nonexistent(self, client, auth_headers):
        resp = client.post(
            "/api/adm1_fail", headers=auth_headers, json={"id": 99999, "user_name": "Admin", "user_number": "admin1"}
        )
        assert resp.status_code in (404, 400)

    def test_adm2_pass_nonexistent(self, client, adm2_headers):
        resp = client.post(
            "/api/adm2_pass", headers=adm2_headers, json={"id": 99999, "user_name": "Admin", "user_number": "admin2"}
        )
        assert resp.status_code in (404, 400)

    def test_adm2_fail_nonexistent(self, client, adm2_headers):
        resp = client.post(
            "/api/adm2_fail", headers=adm2_headers, json={"id": 99999, "user_name": "Admin", "user_number": "admin2"}
        )
        assert resp.status_code in (404, 400)
