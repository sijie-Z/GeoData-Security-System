...
class RecallCreateResource(Resource):
    ...
    def post(self):
        ...
        existing = RecallProposal.query.filter_by(application_id=application_id, status="voting").first()
        if existing:
        ...
        proposal = RecallProposal(
            ...
            reason=reason,
            status="voting",
            voting_deadline=datetime.now(UTC) + timedelta(days=7),
        )

        try:
            db.session.add(proposal)
            db.session.commit()
            log_action(proposer_number, proposer_name, "回收提议创建", "成功", f"app_id={application_id}")
            return {"status": True, "msg": "回收提议创建成功", "data": proposal.to_dict()}, 201
...
class RecallVoteResource(Resource):
    ...
    def post(self, proposal_id):
        ...
        if proposal.voting_deadline and datetime.now(UTC) > proposal.voting_deadline:
        ...
        try:
            if result is True:
                ...
                app.recalled_at = datetime.now(UTC)
            ...
            return {"status": True, "msg": f"投票成功，当前状态: {reason}", "data": proposal.to_dict()}, 200
...
class RecallDetailResource(Resource):
    ...
    def get(self, proposal_id):
        ...
        data["can_vote"] = (
            is_admin_role(identity.get("role"))
            and voter_number != proposal.proposer_number
...
class RecallCloseResource(Resource):
    ...
    def post(self, proposal_id):
        ...
        try:
            ...
            log_action(
                identity.get("number"),
                identity.get("username", "admin"),
                "回收投票关闭",
                "成功",
                f"proposal_id={proposal_id} result={result}",
            )

            return {"status": True, "msg": f"投票已结束: {reason}", "data": proposal.to_dict()}, 200
...
class RecallRestoreResource(Resource):
    """恢复已回收的数据"""
    ...
    def post(self):
        identity = get_jwt_identity()
        if not is_admin_role(identity.get("role")):
            return {"status": False, "msg": "只有管理员可以恢复数据"}, 403
        ...
        if not app.is_recalled:
            return {"status": False, "msg": "该数据未被回收"}, 400

        reason = (data.get("reason") or "").strip()
        if len(reason) < 10:
            return {"status": False, "msg": "恢复原因至少需要10个字符"}, 400

        try:
            app.is_recalled = False
            app.download_enabled = True
            app.recall_reason = None
            app.recalled_at = None

            notification = EmployeeNotification(
                user_number=app.applicant_user_number,
                title="数据恢复通知",
                content=f"您的数据申请(ID: {app.id})已恢复。原因: {reason}",
                sender_number="system",
                sender_name="系统",
            )
            db.session.add(notification)
            ...
            log_action(
                ...
                "数据恢复",
                "成功",
                f"app_id={application_id} reason={reason}",
            )

            return {"status": True, "msg": "数据已恢复", "data": app.to_dict()}, 200
        except Exception as e:
            db.session.rollback()
            logging.error(str(e))
...
class RecallHistoryResource(Resource):
    ...
    def get(self, application_id):
        proposals = (
            RecallProposal.query.filter_by(application_id=application_id)
            .order_by(RecallProposal.created_at.desc())
            .all()
        )
        return {"status": True, "data": [p.to_dict() for p in proposals]}, 200
