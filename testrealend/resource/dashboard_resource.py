...
def _day_window(days_ago):
    end = datetime.now(UTC).replace(hour=0, minute=0, second=0, microsecond=0) + timedelta(days=1 - days_ago)
...
class AdminDashboardResource(Resource):
    ...
    def get(self):
        ...
        active_users_today = (
            db.session.query(func.count(func.distinct(LogInfo.user_number)))
            .filter(LogInfo.timestamp >= today_start, LogInfo.action.in_(["login", "download", "apply", "view"]))
            .scalar()
            or 0
        )

        # 今日登录数
        logins_today = LogInfo.query.filter(LogInfo.action == "login", LogInfo.timestamp >= today_start).count()

        # 今日申请数
        applications_today = Application.query.filter(Application.application_submission_time >= today_start).count()

        # 今日下载数
        downloads_today = DownloadRecord.query.filter(DownloadRecord.timestamp >= today_start).count()
        ...
        for admin in admins:
            ...
            approved_count = Application.query.filter(
                or_(Application.adm1_user_number == admin.adm_number, Application.adm2_user_number == admin.adm_number),
        ...
        hot_data = (
            db.session.query(
                Application.data_name, Application.data_type, func.count(DownloadRecord.id).label("download_count")
            )
            .join(DownloadRecord, DownloadRecord.application_id == Application.id)
            .group_by(Application.data_name, Application.data_type)
        ...
        hot_data_list = [
            {"name": row[0] or "未知数据", "type": row[1] or "unknown", "downloads": row[2]} for row in hot_data
        ]

        # ========== 申请类型分布 ==========
        type_distribution = (
            db.session.query(Application.data_type, func.count(Application.id).label("count"))
            .group_by(Application.data_type)
            .all()
        )
        ...
        for hour in range(24):
            ...
            hourly_activity.append({"hour": f"{hour:02d}:00", "count": activity_count})

        # ========== 本周 vs 上周对比 ==========
        this_week_start = datetime.now(UTC) - timedelta(days=datetime.now(UTC).weekday())
        this_week_start = this_week_start.replace(hour=0, minute=0, second=0, microsecond=0)
        last_week_start = this_week_start - timedelta(days=7)
        last_week_end = this_week_start

        this_week_apps = Application.query.filter(Application.application_submission_time >= this_week_start).count()
        ...
        this_week_downloads = DownloadRecord.query.filter(DownloadRecord.timestamp >= this_week_start).count()

        last_week_downloads = DownloadRecord.query.filter(
            DownloadRecord.timestamp >= last_week_start, DownloadRecord.timestamp < last_week_end
        ).count()

        # 计算增长率
        app_growth = ((this_week_apps - last_week_apps) / last_week_apps * 100) if last_week_apps > 0 else 0
        download_growth = (
            ((this_week_downloads - last_week_downloads) / last_week_downloads * 100) if last_week_downloads > 0 else 0
        )
        ...
        if active_users_today == 0:
            alerts.append({"level": "info", "type": "no_activity", "message": "今日尚无用户活动记录"})
        ...
        for i in range(13, -1, -1):
            date = (datetime.now(UTC) - timedelta(days=i)).strftime("%m-%d")
            start, end = _day_window(i)

            app_count = Application.query.filter(
                Application.application_submission_time >= start, Application.application_submission_time < end
            ).count()

            dl_count = DownloadRecord.query.filter(
                DownloadRecord.timestamp >= start, DownloadRecord.timestamp < end
            ).count()

            login_count = LogInfo.query.filter(
                LogInfo.action == "login", LogInfo.timestamp >= start, LogInfo.timestamp < end
            ).count()

            daily_trend.append({"date": date, "applications": app_count, "downloads": dl_count, "logins": login_count})

        # ========== 状态分布 ==========
        status_distribution = {
            "pending": pending_applications,
            "approved": Application.query.filter(
                Application.adm1_statu == True, Application.adm2_statu == True
            ).count(),
        ...
        for log in logs:
            recent_logs.append({"action": log.action, "username": log.username, "timestamp": log.timestamp.isoformat()})
        ...
        for i in range(5, -1, -1):
            ...
            if i > 0:
                month_end = (datetime.now(UTC) - timedelta(days=(i - 1) * 30)).replace(day=1)
            else:
                month_end = datetime.now(UTC) + timedelta(days=1)
            ...
            month_downloads = DownloadRecord.query.filter(
                DownloadRecord.timestamp >= month_start, DownloadRecord.timestamp < month_end
            ).count()

            monthly_stats.append(
                {"month": month_start.strftime("%Y-%m"), "applications": month_apps, "downloads": month_downloads}
            )
        ...
        return {
            ...
            "data": {
                ...
                # 数据类型统计
                "data_type_stats": {"vector": vector_count, "raster": raster_count},
                # 最近日志
...
class EmployeeDashboardResource(Resource):
    ...
    def get(self):
        ...
        downloadable = Application.query.filter_by(
            applicant_user_number=user_number, adm1_statu=True, adm2_statu=True
        ).count()
        ...
        today_visits = LogInfo.query.filter(
            LogInfo.user_number == user_number, LogInfo.action == "login", LogInfo.timestamp >= today_start
        ).count()

        last_login = (
            LogInfo.query.filter(LogInfo.user_number == user_number, LogInfo.action == "login")
            .order_by(LogInfo.timestamp.desc())
            .first()
        )

        # 我的数据偏好
        vector_prefs = (
            Application.query.filter_by(applicant_user_number=user_number)
            .filter(db.func.lower(Application.data_type) == "vector")
            .count()
        )
        raster_prefs = (
            Application.query.filter_by(applicant_user_number=user_number)
            .filter(db.func.lower(Application.data_type) == "raster")
            .count()
        )
        ...
        recent_apps = (
            Application.query.filter_by(applicant_user_number=user_number)
            .order_by(Application.application_submission_time.desc())
            .limit(5)
            .all()
        )
        ...
        recent_applications = [
            {
                ...
                "status": "已通过"
                if (app.adm1_statu and app.adm2_statu)
                else ("已拒绝" if (app.adm1_statu == False or app.adm2_statu == False) else "待审批"),
                "submission_time": app.application_submission_time.strftime("%Y-%m-%d %H:%M")
                if app.application_submission_time
                else "-",
            }
        ...
        for i in range(13, -1, -1):
            ...
            daily_trend.append({"date": date, "applications": app_count, "downloads": dl_count})

        # 申请状态分布
        status_dist = {
            "pending": pending_apps,
            "approved": downloadable,
            "rejected": Application.query.filter_by(applicant_user_number=user_number)
            .filter(or_(Application.adm1_statu == False, Application.adm2_statu == False))
            .count(),
        }
        ...
        return {
            ...
            "data": {
                ...
                "last_login_time": last_login.timestamp.strftime("%Y-%m-%d %H:%M")
                if last_login
                else datetime.now(UTC).strftime("%Y-%m-%d %H:%M"),
                "data_type_preference": {"vector": vector_prefs, "raster": raster_prefs},
