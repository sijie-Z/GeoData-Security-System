...
class GetEmpInfoListResource(Resource):
    ...
    def get(self):
        ...
        return {"status": True, "msg": "记录获取成功", "data": {"list": data_list, "rows": pagination.total}}, 200
    ...
    def put(self, employee_number):
        try:
            ...
            if job_number is not None:
                ...
                dup = EmployeeInfo.query.filter(
                    EmployeeInfo.job_number == target, EmployeeInfo.employee_number != employee_number
                ).first()
                if dup:
                    return {"status": False, "msg": "工号已存在"}, 400
            ...
            if id_number is not None:
                ...
                dup = EmployeeInfo.query.filter(
                    EmployeeInfo.id_number == target, EmployeeInfo.employee_number != employee_number
                ).first()
                if dup:
                    return {"status": False, "msg": "身份证号已存在"}, 400
            ...
            if phone_number is not None:
                ...
                dup = EmployeeInfo.query.filter(
                    EmployeeInfo.phone_number == target, EmployeeInfo.employee_number != employee_number
                ).first()
                if dup:
                    return {"status": False, "msg": "手机号已存在"}, 400
