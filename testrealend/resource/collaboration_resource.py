...
from utils.websocket import notify_message_read, notify_new_chat_message
...
class ProtectedResource(Resource):
    ...
    def get(self):
        ...
        display_name = _display_name(number, role)
        return {"status": True, "data": {"user_number": number, "role": role, "user_name": display_name}}, 200
    ...
    def post(self):
        ...
        records = [
            EmployeeNotification(
                user_number=n, title=title, content=content, sender_number=sender_number, sender_name=sender_name
            )
            for n in recipients
        ]
        if records:
...
class EmployeeNotificationsResource(Resource):
    ...
    def get(self):
        ...
        pagination = query.order_by(EmployeeNotification.created_at.desc()).paginate(
...
class AdminAnnouncementManageResource(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            item = Announcement(title=title, content=content, author_id=(data.get("author_id") or ""))
            if hasattr(item, "tag"):
...
class ChatSearchUsersResource(Resource):
    ...
    def get(self):
        ...
        data = [
            {"number": i.employee_number, "name": i.name, "role": "employee"}
            for i in emps
            if i.employee_number != me_number
        ] + [{"number": i.adm_number, "name": i.name, "role": "admin"} for i in adms if i.adm_number != me_number]
...
class ChatFriendRequestsResource(Resource):
    ...
    def get(self):
        me_number, me_role, _ = _me()
        rows = (
            FriendRequest.query.filter_by(friend_number=me_number, friend_role=me_role, status="pending")
...
class ChatFriendRespondResource(Resource):
    ...
    def post(self):
        ...
        req = FriendRequest.query.filter_by(
            id=request_id, friend_number=me_number, friend_role=me_role, status="pending"
        ).first()
        if not req:
...
class BatchReviewFailedExportResource(Resource):
    ...
    def post(self):
        ...
        rows = []
        for item in failed:
            rows.append({"id": item.get("id", ""), "reason": item.get("reason", ""), "msg": item.get("msg", "")})
        csv_str = "id,reason,msg\n"
        for r in rows:
            csv_str += f"{r['id']},{str(r['reason']).replace(',', '，')},{str(r['msg']).replace(',', '，')}\n"
        text_buffer.write(csv_str.encode("utf-8-sig"))
        text_buffer.seek(0)
        return send_file(text_buffer, mimetype="text/csv", as_attachment=True, download_name="batch_review_failed.csv")
...
class AdminUserListResource(Resource):
    ...
    def get(self):
        ...
        emp_list = [{"employee_number": e.employee_number, "name": e.name, "role": "employee"} for e in employees]

        adm_list = [{"adm_number": a.adm_number, "name": a.name, "role": "admin"} for a in admins]

        return {"status": True, "data": {"employees": emp_list, "admins": adm_list}}, 200
