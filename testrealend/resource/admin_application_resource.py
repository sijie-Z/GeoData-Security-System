...
class AdminApplicationEligibilityResource(Resource):
    ...
    def get(self):
        ...
        if is_admin_role(role):
            return {"status": True, "data": {"eligible": False, "reason": "您已经是管理员"}}, 200

        # Get employee info
        emp = EmployeeInfo.query.filter_by(employee_number=user_number).first()
        if not emp:
            return {"status": True, "data": {"eligible": False, "reason": "员工信息不存在"}}, 200

        # Check registration time (7+ days)
        if not emp.create_time:
            return {"status": True, "data": {"eligible": False, "reason": "无法确定注册时间"}}, 200

        _ct = emp.create_time.replace(tzinfo=UTC) if emp.create_time.tzinfo is None else emp.create_time
        days_registered = (datetime.now(UTC) - _ct).days
        ...
        pending = (
            AdminApplication.query.filter_by(employee_number=user_number)
            .filter(AdminApplication.status.in_(["pending", "voting"]))
            .first()
        )
        ...
        if pending:
            return {
                "status": True,
                "data": {"eligible": False, "reason": "您已有待处理的申请", "pending_application": pending.to_dict()},
            }, 200

        return {
            "status": True,
            "data": {"eligible": True, "reason": "符合申请条件", "days_registered": days_registered},
        }, 200
...
class AdminApplicationSubmitResource(Resource):
    ...
    def post(self):
        ...
        application = AdminApplication(
            employee_number=user_number, employee_name=emp.name, reason=reason, status="pending"
        )
        ...
        try:
            ...
            return {"status": True, "msg": "申请已提交，等待管理员审核", "data": application.to_dict()}, 201
...
class AdminApplicationMyResource(Resource):
    ...
    def get(self):
        ...
        return {"status": True, "data": [app.to_dict() for app in applications]}, 200
...
class AdminApplicationListResource(Resource):
    ...
    def get(self):
        ...
        return {
            "status": True,
            "data": {"list": [app.to_dict() for app in pagination.items], "total": pagination.total},
        }, 200
...
class AdminApplicationDetailResource(Resource):
    ...
    def get(self, application_id):
        ...
        data["can_vote"] = is_admin_role(identity.get("role")) and application.status in ["pending", "voting"]
    ...
    def post(self, application_id):
        ...
        try:
            if result is True:
                ...
                emp_account = EmployeeAccount.query.filter_by(employee_number=application.employee_number).first()
                ...
                if emp_account:
                    ...
                    # Create admin info
                    emp_info = EmployeeInfo.query.filter_by(employee_number=application.employee_number).first()
                    if emp_info:
                        adm_info = AdmInfo(
                            adm_number=application.employee_number, name=emp_info.name, create_time=datetime.now(UTC)
                        )
            ...
            return {"status": True, "msg": f"投票成功，{reason}", "data": application.to_dict()}, 200
...
class AdminApplicationCloseResource(Resource):
    ...
    def post(self, application_id):
        ...
        try:
            ...
            return {"status": True, "msg": f"审核已结束: {reason}", "data": application.to_dict()}, 200
