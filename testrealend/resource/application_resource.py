...
def _to_dual_channel_dict(item):
    return {
        ...
        "reason": item.reason,
        "application_time": item.application_submission_time.strftime("%Y-%m-%d %H:%M:%S")
        if item.application_submission_time
        else None,
        ...
        "first_comment": item.adm1_comment,
        ...
        "second_comment": item.adm2_comment,
...
def _update_review_result(item, stage, passed, user_name, user_number, comment=None):
    if stage == "adm1":
        item.adm1_statu = bool(passed)
        item.adm1_name = user_name
        item.adm1_user_number = user_number
        item.adm1_approval_time = datetime.now(UTC)
        item.adm1_comment = comment
        return
    item.adm2_statu = bool(passed)
    item.adm2_name = user_name
    item.adm2_user_number = user_number
    item.adm2_approval_time = datetime.now(UTC)
    item.adm2_comment = comment
...
class SubmitApplicationResource(Resource):
    ...
    def post(self):
        ...
        new_app = Application(
            ...
            application_submission_time=datetime.now(UTC),
        )
        try:
...
class WithdrawApplicationResource(Resource):
    ...
    def put(self, application_id):
        ...
        item.recalled_at = datetime.now(UTC)
        item.recall_reason = "申请人主动撤回"
        db.session.commit()

        log_action(
            user_number, item.applicant_name, "撤回申请", "成功", f"app_id={item.id} data_alias={item.data_alias}"
        )
        invalidate_prefix("dashboard")
...
class GetApplicationsResource(Resource):
    ...
    def get(self):
        ...
        if not user_number:
            return {"status": False, "msg": "未提供用户编号"}, 400
        ...
        return {"status": True, "emp_get_applications": items, "pages": {"total": pagination.total}}, 200
...
class Adm2GetApprovedResource(Resource):
    ...
    def get(self):
        ...
        return {"status": True, "approved_application_data": items, "pages": {"total": pagination.total}}, 200
...
class Adm1PassResource(Resource):
    ...
    def post(self):
        ...
        if item:
            ...
            _update_review_result(item, "adm1", True, user_name, user_number, comment)
            db.session.commit()
            log_action(user_number, user_name, "一审通过", "成功", f"app_id={item.id} data_alias={item.data_alias}")
...
class Adm1FailResource(Resource):
    ...
    def post(self):
        data = request.get_json() or {}
        app_id = data.get("id") or data.get("application_id")
        comment = data.get("comment") or data.get("fail_reason", "").strip()
        item = db.session.get(Application, app_id)
        if item:
            ok, msg = _validate_stage_transition(item, "adm1")
            if not ok:
                return {"status": False, "msg": msg}, 400
            user_name, user_number = _review_actor()
            _update_review_result(item, "adm1", False, user_name, user_number, comment)
            db.session.commit()
            log_action(user_number, user_name, "一审驳回", "成功", f"app_id={item.id} data_alias={item.data_alias}")
    ...
    def post(self):
        ...
        if item:
            ...
            if not ok:
                return {"status": False, "msg": msg}, 400
            user_name, user_number = _review_actor()
            _update_review_result(item, "adm2", True, user_name, user_number, comment)
            db.session.commit()
            log_action(user_number, user_name, "二审通过", "成功", f"app_id={item.id} data_alias={item.data_alias}")
    ...
    def post(self):
        ...
        if item:
            ...
            if not ok:
                return {"status": False, "msg": msg}, 400
            user_name, user_number = _review_actor()
            _update_review_result(item, "adm2", False, user_name, user_number, comment)
            db.session.commit()
            log_action(user_number, user_name, "二审驳回", "成功", f"app_id={item.id} data_alias={item.data_alias}")
...
class BatchReviewResource(Resource):
    ...
    def post(self):
        ...
        return {"status": True, "msg": msg, "updated_ids": updated, "skipped": skipped}, 200
...
class ApplicationQRCodeResource(Resource):
    ...
    def get(self, application_id):
        ...
        qr_base64 = Application._normalize_qrcode(item.qrcode)
        if not qr_base64:
...
class ApplicationQRCodeImageResource(Resource):
    ...
    def get(self, application_id):
        ...
        try:
            ...
            logging.warning(f"Invalid base64 qrcode for application {application_id}")
            return {"status": False, "msg": "二维码数据损坏"}, 500
...
class ApplicationDetailResource(Resource):
    ...
    def get(self, application_id):
        ...
        # 添加下载记录
        downloads = (
            DownloadRecord.query.filter_by(application_id=item.id).order_by(DownloadRecord.download_time.desc()).all()
        )
    ...
    def get(self):
        ...
        if status:
            if status == "pending":
                ...
                query = query.filter(db.or_(Application.adm1_statu == False, Application.adm2_statu == False))
        ...
        return {
            "status": True,
            "data": {"list": items, "total": pagination.total, "page": page, "page_size": page_size},
        }, 200
