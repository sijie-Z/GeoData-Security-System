...
class Application(db.Model):
    ...
    adm1_comment = db.Column(db.Text, nullable=True)
    ...
    adm2_comment = db.Column(db.Text, nullable=True)
    ...
    # Watermark Data
    qrcode = db.Column("QRcode", db.Text, nullable=True)
    ...
    # Timestamps
    application_submission_time = db.Column(db.DateTime, default=lambda: datetime.now(UTC))
    ...
    @staticmethod
    def _normalize_qrcode(qr_data):
        """Return a clean base64 string (no data URI prefix) or empty string."""
        if not qr_data:
            return ""
        if isinstance(qr_data, bytes):
            qr_data = base64.b64encode(qr_data).decode("utf-8")
        # Strip any data URI prefix that may have been stored
        if isinstance(qr_data, str) and qr_data.startswith("data:image/"):
            # e.g., "data:image/png;base64,xxxx" → "xxxx"
    ...
    def to_dict(self):
        qr_data = self._normalize_qrcode(self.qrcode)
        ...
        return {
            ...
            "adm1_comment": self.adm1_comment,
            # Second Review
            ...
            "adm2_comment": self.adm2_comment,
            # Watermark
            ...
            # Timestamps
            "application_submission_time": self.application_submission_time.isoformat()
            if self.application_submission_time
            else None,
        }
    ...
    def to_simple_dict(self):
        ...
        return {
            ...
            "is_recalled": self.is_recalled,
            "download_enabled": self.download_enabled,
            "application_time": self.application_submission_time.strftime("%Y-%m-%d %H:%M:%S")
            if self.application_submission_time
            else None,
        }
