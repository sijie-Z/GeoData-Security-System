...
def read_shapefile(shpfile_path):
    encodings = ['gbk', 'iso-8859-1']

    for encoding in encodings:
        try:
            original_shpfile = gpd.read_file(shpfile_path, encoding=encoding)
            print(f"成功使用编码: {encoding}")
            return original_shpfile
        except Exception as e:
            print(f"使用编码 {encoding} 时出错: {e}")

    raise ValueError("无法使用任何指定的编码读取shapefile")
...
def embed(shpfile_path, watermark_path, output_filename):
    ...
    for column in watermarked_shpfile.columns:
        if watermarked_shpfile[column].dtype == 'object':  # 假设 bytes 类型是 object 类型
            watermarked_shpfile[column] = watermarked_shpfile[column].apply(
                lambda x: x.decode('utf-8') if isinstance(x, bytes) else x)

    watermarked_shpfile.to_file(shapefile_path)

    # watermarked_shpfile.to_file(shapefile_path, encoding='utf-8')
...
if __name__ == "__main__":
    # shpfile_path = r"E:\矢量数据\数据\吴江区\WuJiang.shp"
    # shpfile_path = r"E:\矢量数据\数据\River\River.shp"
    # shpfile_path = r"E:\矢量数据\数据\Waterway\Waterway.shp"
    # shpfile_path = r"E:\矢量数据\数据\吴中区\road1.shp"
    # shpfile_path = r"E:\矢量数据\数据\Coastline\Coastline.shp"
    shpfile_path = r"E:\矢量数据\数据\River\River.shp"
    watermark_path = r"C:\Users\kangsumeng\Desktop\下载 (1).png"
