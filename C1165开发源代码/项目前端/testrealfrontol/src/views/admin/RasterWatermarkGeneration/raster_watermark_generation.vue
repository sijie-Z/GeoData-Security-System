...
<template>
  ...
  const get_applications = async () => {
    try {
      // 调用栅格数据专用的API端点
      const response = await axios.get(`${basic_url}/api/adm1_get_raster_applications_generate_watermark`, {
  ...
  const generate = () => {
    requestFormRef.value.validate((valid) => {
      if (valid) {
        // 调用栅格数据生成水印的API端点
        axios.post(`${basic_url}/api/generate_raster_watermark`, requestInformation)
          .then((response) => {
